...
8. Later physical testing found Russian Cyrillic messages classified as Finnish. The whole-chat caller passed no source language to TelegramCore, discarding the chat's known Russian source and producing an unsupported `fi` to `en` request.
9. A mostly-English chat containing Arabic text cached English as its dominant ignored language. That cache suppressed the Arabic translation bar for an hour, and the legacy per-message button preference hid the manual Apple action as well.
...
The later Finnish misclassification was corrected on the `feature/native-privacy-tools` line by moving script-safe source resolution into the audited translation policy and passing the chat's actual source language through the whole-chat local-only request. The fallback applies only when the message contains Cyrillic and the known chat source is also a Cyrillic language; real Finnish Latin text keeps its per-message detection. This follow-up requires a replacement signed-device install and owner-assisted repeat before its physical behavior can be marked passed.

The mixed-language discovery follow-up prefers the strongest non-ignored language in a chat, immediately recomputes ignored cached results, and makes eligible per-message translation available whenever the Apple backend is selected. This allows Arabic in an otherwise English chat to reach Apple without changing the local-only privacy boundary.
